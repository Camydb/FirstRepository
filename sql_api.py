...
def google_db():
    # 打开数据库连接
    db = pymysql.connect(host="database-1.cwrdah6vau2j.us-east-1.rds.amazonaws.com", user="admin", password="Aa123321",
                         port=3306, database="GOOGLE")
    return db
...
def get_share(newsID):
    ...
    db = create_engine(
        'mysql+pymysql://admin:Aa123321@{}:3306/USER'.format("database-1.cwrdah6vau2j.us-east-1.rds.amazonaws.com"))
    df = pd.read_sql('''SELECT user_ID,news_ID,share_ID FROM user_share WHERE news_ID= "{}"  '''.format(newsID),
                     db)  # 分享的新闻，话题，评论
    # db.close()
...
def get_top():
    ...
    # db = google_db()
    db = create_engine(
        'mysql+pymysql://admin:Aa123321@{}:3306/GOOGLE'.format("database-1.cwrdah6vau2j.us-east-1.rds.amazonaws.com"))
    # 查询数据
...
def get_dislike(userID):
    ...
    return (
    tuple((df.loc[df['class_n'] == 'news'])['discuss_ID']), tuple((df.loc[df['class_n'] == 'talk'])['discuss_ID']),
    tuple((df.loc[df['class_n'] == 'discuss'])['discuss_ID']),
    tuple((df.loc[df['class_n'] == 'context'])['discuss_ID']))
...
def get_like(userID):
    ...
    return (tuple((df.loc[df['class_n'] == 'news'])['news_ID']), tuple((df.loc[df['class_n'] == 'talk'])['news_ID']),
            tuple((df.loc[df['class_n'] == 'discuss'])['news_ID']),
            tuple((df.loc[df['class_n'] == 'context'])['news_ID']))
...
#     # cursor = db.cursor()
#     # 使用pandas的read_sql函数读取数据
#     # db = create_engine('mysql+pymysql://admin:Aa123321@{}:3306/GOOGLE'.format("database-1.cwrdah6vau2j.us-east-1.rds.amazonaws.com"))
...
def get_act(search_id, class_n):
    ...
    db = create_engine(
        'mysql+pymysql://admin:Aa123321@{}:3306/USER'.format("database-1.cwrdah6vau2j.us-east-1.rds.amazonaws.com"))
    if class_n == 'news' or class_n == 'talk' or class_n == 'like_discuss':
        df = pd.read_sql('''SELECT user_ID,class_n FROM user_like WHERE news_ID= "{}"  '''.format(search_id),
                         db)  # 收藏的新闻，话题，评论
        # db.close()
        ...
        df = pd.read_sql(
            '''SELECT user_ID,discuss_ID,class_n FROM user_discuss WHERE news_ID= "{}"  '''.format(search_id), db)
        # db.close()
        ...
        df = pd.read_sql(
            '''SELECT user_ID,discuss_ID,class_n FROM user_dislike WHERE discuss_ID= "{}"  '''.format(search_id), db)
        # db.close()
        df = df.drop_duplicates('user_ID')  # .sort_index()
        return (len(df))
    elif class_n == 'share':
        df = pd.read_sql('''SELECT user_ID,news_ID,class_n FROM user_share WHERE news_ID= "{}"  '''.format(search_id),
                         db)
        # db.close()
        # df = df.drop_duplicates('user_ID')#.sort_index()
        return (len(df))
...
def chuan():
    ...
    http://44.201.171.15:5000/chuan?name=safsafsdfsdfsd.png
    '''
    name = request.values.get('name')
    print(name)
    try:
        file = request.files['file']
        path = '/home/ubuntu/img/{}'.format(name)
...
def get_image(name):
    ...
    """
    image_path = os.path.join("/home/ubuntu/img", name)
...
def context():
    ...
    http://44.201.171.15:5000/context?TH_ID=3wPcRUB6R
    '''
    ...
    try:
        ...
        cur.execute(
            'SELECT context,THEME,KEY_WORD,context_ZH,TH_ZH,KW_ZH,CT_ID FROM GL_NEWS WHERE TH_ID = "{}" '.format(TH_ID))
        rows = cur.fetchall()
        # for row in rows:
        #     if row[0] == userID:
        #         path = row[1]
...
def share():
    ...
    http://44.201.171.15:5000/share?userID=fhLgcu7A6RvxkW3sFmN936&newsID=VzLtzi4B8YDFk2SPw37h8s
    '''
...
def get_personal():
    ...
    http://44.201.171.15:5000/personal?userID=Lw7CH4upxDy7tzDS9XChsg
    '''
    userID = request.values.get('userID')
    # get image
    ...
    if path != '':
        ...
        # path = 'D:/image/aa.jpg'
        img = send_file('/home/ubuntu/h_imgs/user.png', mimetype='image/gif')
        # img = send_file('D:/image/aa.jpg', mimetype='image/gif')
        # get data
        data = {
            'user_name': nick_name,
...
def version():
    ...
    http://44.201.171.15:5000/version
    '''
    project = {
        "code": "OK",
        "msg": 'OK',
...
def change_user_info():
    ...
    http://44.201.171.15:5000/change_user_info?userID=adasdsad&user_name=sdfdfsd
    '''
    save_time, stime = get_now_time()
    # path_list = []
    # try:
    ...
    #     for file in files:
    #         path = '/home/ubuntu/images/{}_{}.png'.format(stime, files.index(file))
    #         file.save(path)
    ...
    try:
        file = request.files['file']
        path = '/home/ubuntu/h_imgs/{}.png'.format(stime)
        file.save(path)  # 保存上传的文件到指定路径
    except Exception as e:
        path = ''
        print(e)
    ...
    try:
        ...
        cur.execute(
            '''UPDATE personal_information SET HS_url = "{}",nick_name = "{}"  WHERE app_ID = "{}" '''.format(path,
                                                                                                              user_name,
                                                                                                              userID))
        conn.commit()

        cur.execute('''UPDATE apple_user SET HS_url = "{}",Name = "{}"  WHERE User_ID = "{}" '''.format(path, user_name,
                                                                                                        userID))
        conn.commit()
...
def Projectlist_14():
    ...
    http://44.201.171.15:5000/revise?phone_number=79879879&p_word=123455
    '''
    ...
    try:
        ...
        sql = (''' UPDATE personal_information SET password = "{}" WHERE Phone_number = "{}" '''.format(p_word,
                                                                                                        phone_number))
        cur.execute(sql)
        conn.commit()
...
def Projectlist_13():
    ...
    http://44.201.171.15:5000/cancel?userID=79879879
    '''
    userID = request.values.get('userID')
    try:
        conn = user_db()
...
def Projectlist_12():
    ...
    http://44.201.171.15:5000/del_all?userID=79879879
    '''
    userID = request.values.get('userID')
    try:
        conn = user_db()
...
def upload():
    ...
    http://44.201.171.15:5000/upload?question=scscscscsc&userID=vdfvfdvfdv&class_n=context
    '''
    save_time, stime = get_now_time()
    path_list = []
    try:
    ...
    # try:
    # file = request.files['file']
    # path = '/home/ubuntu/images/{}.png'.format(stime)
    # file.save(path)  # 保存上传的文件到指定路径
    # except Exception as e:
    #     path = ''
    #     print(e)
    ...
    try:
        ...
        cur.execute(
            ...
            VALUES ("{}","{}","{}","{}","{}","{}","{}")'''.format(userID, f_back_ID, question, path_list, save_time,
                                                                  stime, class_n));
        conn.commit()
        print("反馈成功!")
        project = {
...
def Projectlist_11():
    ...
    http://44.201.171.15:5000/add_dislike?userID=2132&newsID=2312312&is_dislike=0&class_n=discuss
    '''
    userID = request.values.get('userID')
    newsID = request.values.get('newsID')
    ...
    if class_n == 'news':
        if is_dislike == 1:
            try:
                cur.execute(
                    '''INSERT INTO user_dislike (user_ID,discuss_ID,class_n) VALUES ("{}","{}","{}")'''.format(userID,
                                                                                                               newsID,
                                                                                                               class_n));
                conn.commit()
                print("记录插入成功!")
    ...
    if class_n == 'talk' or class_n == 'context':
        if is_dislike == 1:
            ...
            sql = '''delete from user_dislike where user_ID="{}" and discuss_ID="{}" and class_n="{}"'''.format(userID,
                                                                                                                newsID,
                                                                                                                class_n)
            cur.execute(sql)
            conn.commit()
...
def Projectlist_10():
    ...
    http://44.201.171.15:5000/discuss_list?newsID=8T4AZAkd5t9bBXG3kFuAUP&class_n=talk&userID=3FtPAHrhhveJGNzC5FaBWv
    '''

    newsID = request.values.get('newsID')
    userID = request.values.get('userID')

    like_news, like_talk, like_discuss, like_context = get_like(userID)
    dislike_news, dislike_talk, dislike_discuss, dislike_context = get_dislike(userID)
...
def Projectlist9():
    ...
    http://44.201.171.15:5000/del_discuss?userID=3FtPAHrhhveJGNzC5FaBWv&discussID=VSDV&class_n=talk
    '''
    userID = request.values.get('userID')
...
def Projectlist8():
    '''
        ...
        http://127.0.0.1:5000/user_discuss?userID=3FtPAHrhhveJGNzC5FaBWv&newsID=VSDV&text=Gwyneth Paltrow denied a man’s accusation&username=User_dadsd&class_n=news
    http://44.201.171.15:5000/user_discuss?userID=3FtPAHrhhveJGNzC5FaBWv&newsID=VSDV&text=Gwyneth Paltrow denied a man’s accusation&username=User_dadsd&class_n=talk
    '''
    userID = request.values.get('userID')
    newsID = request.values.get('newsID')
    ...
    try:
        ...
        cur.execute(
            '''INSERT INTO user_discuss (user_ID,news_ID,discuss_ID,discuss_text,username,time,S_time,class_n) VALUES ("{}","{}","{}","{}","{}","{}","{}","{}")'''.format(
                userID, newsID, discuss_ID, text1, username, time, stime, class_n));
        conn.commit()
        print("记录插入成功!")
        project = {
            "code": "OK",
...
def Projectlist7():
    ...
    http://44.201.171.15:5000/user_like?userID=3FtPAHrhhveJGNzC5FaBWv
    '''
    userID = request.values.get('userID')
    # newsID = request.values.get('newsID')
    ...
    print(like_news, like_talk, like_discuss, like_context)
    print(dislike_news, dislike_talk, dislike_discuss, dislike_context)
    try:
        if len(like_news) > 0 or len(like_talk) > 0:
            ...
            lc = list(like_context)
            # db = google_db()
            cur = db.cursor()

            df = pd.DataFrame(columns=['GPT3_TEXT', 'TIME', 'KEY_WORD', 'S_TIME', 'TITLE', 'NEWS_ID', 'ZH_CN', 'T_ZH',
                                       'KW_ZH'])  # ,index=[]
            # 执行查询语句
            ...
            for row in cur:
                if row[5] in lm:
                    ...
                    df = pd.concat([df, pd.DataFrame(
                        {'GPT3_TEXT': [row[0]], 'TIME': [row[1]], 'KEY_WORD': [row[2]], 'S_TIME': [row[3]],
                         'TITLE': [row[4]], 'NEWS_ID': [row[5]], 'ZH_CN': [row[6]], 'T_ZH': [row[7]],
                         'KW_ZH': [row[8]]})], ignore_index=True)
            ...
            df = pd.DataFrame(
                columns=["THEME", "GPT3_TITLE", "KEY_WORD", "S_TIME", "TIME", "TH_ID", "TH_ZH", "GT_ZH", "KW_ZH","AI_img"])
            # 执行查询语句
            cur.execute('SELECT THEME,GPT3_TITLE,KEY_WORD,S_TIME,TIME,TH_ID,TH_ZH,GT_ZH,KW_ZH,AI_img FROM GL_NEWS')
            # 遍历查询结果并保存到 Pandas 数据框中
            for row in cur:
                if row[5] in lt:
                    ...
                    df = pd.concat([df, pd.DataFrame(
                        {'THEME': [row[0]], 'GPT3_TITLE': [row[1]], 'KEY_WORD': [row[2]], 'S_TIME': [row[3]],
                         'TIME': [row[4]], 'TH_ID': [row[5]], 'TH_ZH': [row[6]], 'GT_ZH': [row[7]],
                         'KW_ZH': [row[8]], 'AI_img': [row[9]]})], ignore_index=True)
            ...
            if len(df) > 0:
                for index, row in df.iterrows():
                    ...
                    if row['AI_img'] == '':
                        ...
                        aimg = "http://44.201.171.15:5000/images/" + row['AI_img']
                    talks = {
                        "theme": b,
            ...
            for row in cur:
                if row[0] in ld:
                    ...
                    df = pd.concat([df, pd.DataFrame(
                        {'discuss_ID': [row[0]], 'discuss_text': [row[1]], 'username': [row[2]], 'time': [row[3]],
                         'S_time': [row[4]], 'user_ID': [row[5]]})], ignore_index=True)
            ...
            df = pd.DataFrame(columns=['context', 'context_ZH', 'CT_ID', 'S_TIME', 'TIME'])
            # 执行查询语句
            cur.execute('SELECT context,context_ZH,CT_ID,S_TIME,TIME FROM GL_NEWS')
            # 遍历查询结果并保存到 Pandas 数据框中
            for row in cur:
                if row[2] in lc:
                    # df = df.append({'context': row[0], 'context_ZH': row[1], 'CT_ID': row[2], 'S_TIME': row[3], 'TIME': row[4]}, ignore_index=True)
                    df = pd.concat([df, pd.DataFrame(
                        {'context': [row[0]], 'context_ZH': [row[1]], 'CT_ID': [row[2]], 'S_TIME': [row[3]],
                         'TIME': [row[4]]})], ignore_index=True)

            df = df.sort_values('S_TIME', ascending=False).drop_duplicates('CT_ID')  # .sort_index()
            db.close()
            context_list = []
            if len(df) > 0:
                for index, row in df.iterrows():
                    like_sum = get_act(row['CT_ID'], 'news')
                    discuss_sum = get_act(row['CT_ID'], 'discuss')
                    dislike_sum = get_act(row['CT_ID'], 'dislike')

                    dislike = False
                    if row['CT_ID'] in dislike_context:
                        dislike = True
                    # st = get_max_time(b)
                    news = {
                        "context": row['context'],
                        "context_ZH": row['context_ZH'],
                        "CT_ID": row['CT_ID'],
                        "S_TIME": row['S_TIME'],
                        "TIME": row['TIME'],
                        "like": True,
                    ...
                    if news not in context_list:
                        context_list.append(news)
...
def Projectlist6():
    ...
    http://44.201.171.15:5000/add_like?userID=2132&newsID=2312312&is_like=0&class_n=news
    '''
    userID = request.values.get('userID')
    newsID = request.values.get('newsID')
    ...
    if class_n == 'news':
        if is_like == 1:
            try:
                cur.execute(
                    '''INSERT INTO user_like (user_ID,news_ID,class_n) VALUES ("{}","{}","{}")'''.format(userID, newsID,
                                                                                                         class_n));
                conn.commit()
                print("记录插入成功!")
                project = {
    ...
    if class_n == 'talk' or class_n == 'context':
        if is_like == 1:
            ...
            sql = '''delete from user_like where user_ID="{}" and news_ID="{}" and class_n="{}"'''.format(userID,
                                                                                                          newsID,
                                                                                                          class_n)
            cur.execute(sql)
            conn.commit()
...
def Projectlist5():
    ...
    http://44.201.171.15:5000/apple_login?appleID=123123&name=lily&email=789@qq.com&Acode=963258&token=wqeq&RUS=2
    '''
        ...
        if appleID == None:
            ...
            if aid in rest:
                ...
                sql = """INSERT INTO apple_user (User_ID,Apple_ID,Real_Name,Name,Email,A_Code,Token,RUS,SEX,Status)
                         VALUES ("{}","{}","{}","{}","{}","{}","{}","{}","{}","{}")""".format(app_ID, appleID, name,
                                                                                              n_name, email, Acode,
                                                                                              token, RUS, 1, 1)
                try:
                    # 执行sql语句
...
def Projectlist4():
    ...
    http://44.201.171.15:5000/quick_login?number=15915278761
    :return:
    '''
...
def Projectlist3():
    ...
    http://44.201.171.15:5000/login?number=15915278761&password=AA123456
    :return:
    '''
    num = request.values.get('number')
    password = request.values.get('password')
...
def Projectlist2():
    ...
    http://44.201.171.15:5000/register?number=15915278761&password=AA123456
    :return:
    '''
    num = request.values.get('number')
    try:
        if num == None:
            ...
            if result1:
                ...
                sql = """INSERT INTO personal_information (app_ID,HS_url,real_name,ID_Number,nick_name,SEX,Phone_number,password,Status)
                         VALUES ("{}","{}","{}","{}","{}","{}","{}","{}","{}")""".format(U["app_ID"], U["HS_url"],
                                                                                         U["real_name"], U["ID_Number"],
                                                                                         U["nick_name"],
                                                                                         U["SEX"], U["Phone_number"],
                                                                                         U["password"], U["Status"])
                try:
                    # 执行sql语句
                    cursor.execute(sql)
...
def Projectlist1():
    ...
    http://44.201.171.15:5000/search/phone?number=15915278761
    '''
    num = request.values.get('number')
    # name= request.values.get('name')
    try:
...
def G_topic_today():
    ...
    http://44.201.171.15:5000/topic_today?page=1&userID=3FtPAHrhhveJGNzC5FaBWv
    '''
    ...
    try:
        ...
        # sql = "select THEME,GPT3_TITLE,KEY_WORD from GL_NEWS"
        df = pd.read_sql(
            "select THEME,GPT3_TITLE,KEY_WORD,S_TIME,TIME,TH_ID,TH_ZH,GT_ZH,KW_ZH,AI_img,PIC_URL,REMARKS from GL_NEWS", db)
        db.close()
        df = df.sort_values('S_TIME', ascending=False).drop_duplicates('TH_ID')  # .sort_index()
        # print(df)
        order = get_top()
        ...
        if page <= total_pages:
            ...
            for index, row in df.iterrows():
                ...
                if row['AI_img'] == '':
                    ...
                    aimg = "http://44.201.171.15:5000/images/" + row['AI_img']
                strings = row['KEY_WORD'].replace(".", "").replace(" ", "").split(",")
                new_strings = ["#" + s for s in strings]

                strings = remove_text(row['KW_ZH']).replace(".", "").replace(" ", "").split(",")
                zh_strings = ["#" + s for s in strings]
                category = "world"
                category_ZH = "全球"
                news = {
                    "theme": row['THEME'],
...
def G_news_lists():
    '''
        http://127.0.0.1:5000/news_list?word=Ramadan Fasting to Begin on Thursday in Muslim Countries and UK&li=1&userID=3FtPAHrhhveJGNzC5FaBWv
    http://44.201.171.15:5000/news_list?word=Ramadan Fasting to Begin on Thursday in Muslim Countries and UK&li=1&userID=3FtPAHrhhveJGNzC5FaBWv
    '''
    ...
    try:
        ...
        sql = '''select GPT3_TEXT,TIME,KEY_WORD,S_TIME,TITLE,NEWS_ID,ZH_CN,T_ZH,KW_ZH from GL_NEWS WHERE THEME like "%{}%"  '''.format(
            word)
        df = pd.read_sql(sql, db)
        db.close()
...
def G_twitter():
    '''
        http://127.0.0.1:5000/twitter?word=4t9oarBm9
    http://44.201.171.15:5000/twitter?word=4t9oarBm9
    '''
    word = request.values.get('word')
    print(word)
    try:
...
def P_twitter():
    ...
    http://44.201.171.15:5000/twitter_pl?key_word=NATO Membership
    '''
...
if __name__ == "__main__":
    ...
    server.run(host="172.31.82.161", port=5000, debug=True)
