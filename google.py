    ...
    if wbdata.status_code == 200:
        ...
        for j in js:
            try:
                ...
                # print(j_title)
                # print(j_url)
                get_news(title,new_url,new_time,two,one_title,url,three,location,CLASS_ID)
                # time.sleep(3)
                # print(uu.href)
            except Exception as e:
                print(e)
...
def save(db,TITLE,AUTHOR,CONTENT,TIME,THEME,THEME_URL,GPT3_TITLE,GPT3_TEXT,NEWS_URL,PIC_URL,REMARKS,three,TH_ID):
    ...
    try:
        ...
        TH_ZH = lark(THEME)
        KW_ZH = lark(three)
        GT_ZH = lark(GPT3_TITLE)
        ZH_CN =  lark(GPT3_TEXT)
        print("翻译完成")
        ...
        if "china" in sensitive or "taiwan" in sensitive or "jinping" in sensitive:
            ...
            cur.execute(
                '''INSERT INTO GL_NEWS (NEWS_ID,TITLE,AUTHOR,CONTENT,TIME,S_TIME,SAVE_TIME,THEME,KEY_WORD,THEME_URL,GPT3_TITLE,GPT3_TEXT,NEWS_URL,PIC_URL,REMARKS,TH_ID,T_ZH,TH_ZH,KW_ZH,GT_ZH,ZH_CN) 
                VALUES ("{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}","{}")'''.format(NEWS_ID,TITLE,AUTHOR,CONTENT,TIME,S_TIME,SAVE_TIME,THEME,three,THEME_URL,GPT3_TITLE,GPT3_TEXT,NEWS_URL,PIC_URL,REMARKS,TH_ID,T_ZH,TH_ZH,KW_ZH,GT_ZH,ZH_CN));
            conn.commit()
            print ("记录插入成功!")
...
def get_news(title,news_url,new_time,two,one_tit,one_url,three,location,TH_ID):
    ...
    if news_title not in sql_title:
        ...
        if news_text!='':
            try:
                save("D:/T_py/GOOGLE.db",news_title,news.authors,news_text,new_time,one_tit,one_url,two,gpt_text,news_url,news.top_image,location,three,TH_ID)
...
texts = []
for row in cur.execute('SELECT THEME_URL FROM Gl_NEWS' ):
