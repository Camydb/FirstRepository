# -*- coding: utf-8 -*-
"""
Created on Tue Apr 18 14:11:15 2023
...
def open_db():
    # 打开数据库连接
    db = pymysql.connect(host="120.48.49.157", user="root", password="root123321", port=3306, database="GOOGLE")
    return db
...
cur.execute("SELECT THEME,TH_ZH FROM GL_NEWS")
rows = cur.fetchall()
i=0
for row in rows:
    if row[0] not in texts:
        ...
        try:
            if row[1].startswith("“") and row[1].endswith("”"):
                ...
                cur1.execute('''UPDATE GL_NEWS SET TH_ZH = "{}" WHERE THEME = "{}" '''.format(r,row[0]))
            ...
            #     cur1.execute('''UPDATE GL_NEWS SET TH_ZH = "{}" WHERE THEME = "{}" '''.format(T_ZH,row[0]))
            #     # cur1.execute('''UPDATE Gl_NEWS SET context = "{}" WHERE GPT3_TITLE = "{}" '''.format(r1,row[0]))
            #     db.commit()
            #     print('插入成功')
            #     print(i)
